...
#Create ngrams
ngd_txt['ngrams'] = ngd_txt.apply(lambda row: list(nltk.trigrams(row['txt_tokenized'])),axis=1) 
...
ngd_txt['ngram2'] = ngd_txt.ngrams.apply(lambda row:['_'.join(i) for i in row])
ngd_txt['txt_tokenized2'] = ngd_txt['txt_tokenized'].apply(' '.join)
ngd_txt.head()


# %% Convert trigram lists to strings
ngd_txt['ngram2'] = ngd_txt.ngram2.apply(lambda x: ' '.join([str(i) for i in x]))
ngd_txt.head()
...
word_counts_dict = dict(zip(word_counts['ngram'].tolist(),word_counts['count'].tolist()))

#make the wordcloud
cloud = WordCloud(width = 1600, height = 800, max_font_size=100,max_words = 100, background_color = 'white', relative_scaling = 1, colormap ="viridis_r").generate_from_frequencies(word_counts_dict)
...
def topic_generator(number_topics, number_words, data, count_vectorizer):
...
topic_generator(10,10,ngd_txt['txt_tokenized2'],count_vectorizer)
