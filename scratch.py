...
vitals = vitals.drop(columns=['age_indicator','bp_comment','pulse_pattern', 'BP'])
...
# %%
dfs = [encounters, cpt_codes, vitals, meds, labs, diagnoses]
for df in dfs:
    print(df.isna().sum())
    print(len(df))
    print('\n')
...
# %%
list(diagnoses.description.unique())
# %%
